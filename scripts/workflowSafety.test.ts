...
describe('GitHub Actions data safety', () => {
  ...
  it('backfill 先產生 artifact、驗證 252 筆且只提交 history', () => {
    ...
    expect(backfillWorkflow).toContain('Require stable live update schedule')
    expect(backfillWorkflow).toContain('--event schedule')
    expect(backfillWorkflow).toContain('data:check-stability')
